...
def editar_permisos(request, pk):
    ...
    if request.method == 'POST':
        ...
        if nuevo_rol in [r[0] for r in ROLES_DISPONIBLES]:
            usuario.rol = nuevo_rol
            usuario.save()

        # Actualizar permisos
        permisos.puede_inscribir_alumnos  = 'puede_inscribir_alumnos'  in request.POST
        permisos.puede_generar_pagos      = 'puede_generar_pagos'      in request.POST
        permisos.puede_subir_comunicados  = 'puede_subir_comunicados'  in request.POST
        permisos.puede_subir_horarios     = 'puede_subir_horarios'     in request.POST
        permisos.puede_ver_calificaciones = 'puede_ver_calificaciones' in request.POST
        permisos.puede_gestionar_grupos   = 'puede_gestionar_grupos'   in request.POST
        permisos.puede_ver_reportes       = 'puede_ver_reportes'       in request.POST
        permisos.save()

        messages.success(request, f'Permisos de {usuario.get_full_name()} actualizados.')
        return redirect('lista_personal')

    return render(request, 'users/editar_permisos.html', {
        'usuario':           usuario,
        'permisos':          permisos,
